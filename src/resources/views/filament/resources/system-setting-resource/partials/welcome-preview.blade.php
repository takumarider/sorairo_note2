<div class="space-y-6">
    ...
    <div class="rounded-lg border border-slate-200 bg-white p-4">
        <h3 class="mb-4 text-base font-bold text-slate-900">ウェルカムページプレビュー</h3>
        
        <!-- Preview Container (mimics actual welcome page) -->
        <div class="space-y-4 rounded-lg bg-gradient-to-br from-sky-50 via-white to-cyan-100 p-6">
            ...
            <div class="rounded-2xl bg-white/70 p-6 backdrop-blur-lg">
                <div class="space-y-3">
                    ...
                    @else
                        <p class="text-sm text-slate-500 italic">メイン見出しを入力してください</p>
                    @endif
                    ...
                    @else
                        <p class="text-sm text-slate-500 italic">リード文を入力してください</p>
                    @endif
                    
                    @if ($hero_image ?? null)
                        <div class="mt-4 h-40 overflow-hidden rounded-xl">
                            <img src="{{ $hero_image }}" alt="hero" class="h-full w-full object-cover">
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    @if ($previewBlockCount === 1)
                        @foreach ($body_blocks as $index => $block)
                            <div class="rounded-xl border border-white/60 bg-white/80 p-4 shadow-sm">
                                ...
                                @if ($block['image'] ?? null)
                                    <img src="{{ $block['image'] }}" alt="block" class="mt-3 h-32 w-full rounded-lg object-cover">
                                @endif
                            </div>
                        @endforeach
                    @else
                        ...
                        >
                            ...
                            <div class="mt-3 flex items-center justify-between">
                                <button @click="prev()" :disabled="sliding" class="flex items-center gap-1 rounded-full border border-sky-200 bg-white/80 px-3 py-1.5 text-xs font-semibold text-sky-700 shadow hover:bg-sky-50 transition">
                                ...
                                <button @click="next()" :disabled="sliding" class="flex items-center gap-1 rounded-full border border-sky-200 bg-white/80 px-3 py-1.5 text-xs font-semibold text-sky-700 shadow hover:bg-sky-50 transition">
                                    次へ
        ...
        <p class="mt-4 text-xs text-slate-500">
            ※ このプレビューは編集中の内容をリアルタイムで表示しています。実際の表示確認は保存後に行えます。
        </p>
    </div>
</div>
